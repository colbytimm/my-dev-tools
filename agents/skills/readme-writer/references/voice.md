...
"Sounds AI" is not a vibe; it's a catalog of specific, recognizable patterns that readers have internalized. A README that triggers them costs the project credibility before the reader evaluates the code. This file defines the patterns and how to write instead.

The underlying principle: the best READMEs are written by someone who built the thing, knows exactly what it does, and respects the reader's time. Generated-sounding text fails on all three - it praises instead of describes, decorates instead of informs, and pads instead of stopping.
...
If no concrete fact backs the adjective, the claim doesn't belong in the README.
...
- No emoji in headers. No 🚀 ✨ 🔥 anywhere. (See the emoji policy in SKILL.md for the narrow exception: semantic use like ✅/❌ in a support matrix the README defines.)
...
No "In conclusion", "Overall", "In summary", "To summarize". A README section ends when the information ends. Also cut throat-clearing openers: "In today's fast-paced development world...", "Whether you're a beginner or an expert...". Start with the thing.
...
9. Read the first 10 lines as a stranger. Do you know what the project is, and could you start using it within a minute of scrolling? If not, restructure.
